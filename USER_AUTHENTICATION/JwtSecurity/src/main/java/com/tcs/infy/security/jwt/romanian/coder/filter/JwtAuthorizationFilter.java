...
public class JwtAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    private Authentication getUsernamePasswordAuthentication(HttpServletRequest request) throws  UsernameNotFoundException{
        ...
        if (token != null) {
            ...
            String userName = JWT.require(HMAC512(jwtProperties.getTokenPrefix().getBytes()))
                    .build()
                    .verify(token)
                    .getSubject();
            ...
            if (userName != null) {
                ...
                User user=null;
                User isUserPresent=userRepository.findByUsername(userName).orElse(null);

                if (isUserPresent!=null && isUserPresent.getUsername()!=null && isUserPresent.getUsername().length()>0) {
                    user=isUserPresent;
                } else {
